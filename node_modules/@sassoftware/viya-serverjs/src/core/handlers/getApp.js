...
async function getApp (options, req, h) {
  let authType = options.authFlow;
  if (authType === "implicit") {
    debug("calling handleImplicit");
    return handleImplicit(options, req, h);
  } 
  if (authType === "server") {
    let status = await codeAuth(req, h, options);
    return status;
  } else if (authType === null) { /* case of no auth */;
      let indexHTML = process.env.APPENTRY == null ? "index.html" : process.env.APPENTRY;
      console.log(`Redirecting to default ${indexHTML}`);
      return { status: true, error: null , target: 'file', redirect: indexHTML};
  }
  async function handleImplicit (options, req, h) {
