...
function server(userRouterTable, asset, allAppEnv) {
  ...
  if (process.env.COOKIE != null) {
    var _process$env$COOKIE$s = process.env.COOKIE.split(','),
        _process$env$COOKIE$s2 = _slicedToArray(_process$env$COOKIE$s, 2),
        s1 = _process$env$COOKIE$s2[0],
        s2 = _process$env$COOKIE$s2[1];
