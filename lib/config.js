...
function config(appEnv, dockerFile) {
  ...
  if (vserver != null) {
    var _vserver = _vserver.trim();

    if (_vserver.endsWith('/') === true) {
      _vserver = _vserver.substring(0, _vserver.length - 1);
    }

    if (_vserver.indexOf('http') < 0) {
      _vserver = process.env.SAS_PROTOCOL + _vserver;
    }
  }
