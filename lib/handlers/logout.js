...
function _logout() {
  _logout = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(req, h) {
    var q, credentials, sid, callbackUrl, url, _callbackUrl;
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 9:
            req.cookieAuth.clear('authCookie'); //await ViyaLogout(); 

            debug(req.state);
            callbackUrl = "".concat(req.server.info.uri, "/").concat(process.env.APPNAME, "/").concat(q.callbackUrl);
            url = "".concat(process.env.VIYA_SERVER, "/SASLogon/logout?callbackUrl=").concat(callbackUrl);
            return _context.abrupt("return", h.redirect(url).unstate('authCookie'));

          case 16:
            _callbackUrl = "".concat(req.server.info.uri, "/").concat(process.env.APPNAME, "/").concat(q.callbackUrl);
            return _context.abrupt("return", h.redirect(_callbackUrl));
