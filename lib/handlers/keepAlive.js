...
function _keepAlive() {
  _keepAlive = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(req, h) {
    var credentials, SASLogon;
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            credentials = refreshToken(req, h);
            SASLogon = "".concat(process.env.VIYA_SERVER, "/SASLogon/oauth/authorize?client_id=").concat(process.env.CLIENTID, "&redirect_uri=").concat(process.env.APPSERVER, "/keepAlive2&response_type=code"); // let SASLogon = `${process.env.VIYA_SERVER}/SASLogon/`;
