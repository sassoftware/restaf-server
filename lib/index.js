...
function getAllEnv(userData) {
  ...
  if (authflow === 'server' || authflow === 'implicit') {
    if (authflow === 'implicit') {
      ...
      redirect = redirect == null ? 'callback' : "".concat(process.env.APPNAME, "/").concat(redirect);
    }
    ...
    if (authflow === 'server' && keepAlive !== null) l.keepAlive = "http://".concat(process.env.APPHOST, ":").concat(process.env.APPPORT, "/keepAlive");
