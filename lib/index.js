...
function icli(uTable, useDefault) {
  ...
  console.log("docker: ".concat(docker));
  console.log("env: ".concat(env));
  console.log("appenv: ".concat(appenv));
...
function iapp(appSrc, rafEnv, dockerFile, uTable, useDefault) {
  ...
  console.log("process.env.APPENV = ".concat(process.env.APPENV));
...
function getAllEnv(userData) {
  ...
  if (authflow === 'implicit') {
    l = {
      ...
      redirect: process.env.REDIRECT != null ? "".concat(trimit('APPNAME'), "/").concat(trimit('REDIRECT')) : null,
    ...
    l = {
      ...
      keepAlive: null
      /*`http://${process.env.APPHOST}:${process.env.APPPORT}/keepAlive`*/
